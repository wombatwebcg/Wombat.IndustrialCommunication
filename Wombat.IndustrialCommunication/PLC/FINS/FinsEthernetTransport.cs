...
namespace Wombat.IndustrialCommunication.PLC
{
    ...
    public class FinsEthernetTransport : DeviceMessageTransport
    {
        ...
        public override async Task<OperationResult<IDeviceReadWriteMessage>> UnicastReadMessageAsync(IDeviceReadWriteMessage message)
        {
            ...
            try
            {
                DebugLog("[FINS读取调试] 开始执行读取操作");
                DebugLog("[FINS读取调试] 请求参数: RegisterCount={RegisterCount}, DataType={DataType}, Length={Length}", request.RegisterCount, request.DataType, request.Length);
                DebugLog("[FINS读取调试] 请求命令: {RequestFrame}", string.Join(" ", request.ProtocolMessageFrame.Select(b => b.ToString("X2"))));

                // 发送请求
                DebugLog("[FINS读取调试] 开始发送读取请求");
                var sendResult = await SendRequestAsync(request.ProtocolMessageFrame);
                if (!sendResult.IsSuccess)
                {
                    DebugLog("[FINS读取调试] 发送请求失败: {Message}", sendResult.Message);
                    return OperationResult.CreateFailedResult<IDeviceReadWriteMessage>($"发送FINS读取请求失败: {sendResult.Message}");
                }
                DebugLog("[FINS读取调试] 请求发送成功");

                // 接收FINS响应头部 (10字节)
                DebugLog("[FINS读取调试] 开始接收响应头部 (10字节)");
                var headerResult = await ReceiveResponseAsync(0, FinsConstants.FINS_HEADER_LENGTH);
                if (!headerResult.IsSuccess)
                {
                    DebugLog("[FINS读取调试] 接收头部失败: {Message}", headerResult.Message);
                ...
                DebugLog("[FINS读取调试] 接收到头部: {Header}", string.Join(" ", headerBuffer.Select(b => b.ToString("X2"))));

                // 接收响应码部分 (MRC, SRC) - 2字节
                var responseCodeLength = FinsCommonMethods.GetContentLength(headerBuffer);
                DebugLog("[FINS读取调试] 开始接收响应码 ({ResponseCodeLength}字节)", responseCodeLength);
                var responseCodeResult = await ReceiveResponseAsync(0, responseCodeLength);
                if (!responseCodeResult.IsSuccess)
                {
                    DebugLog("[FINS读取调试] 接收响应码失败: {Message}", responseCodeResult.Message);
                    return OperationResult.CreateFailedResult<IDeviceReadWriteMessage>($"接收FINS读取响应码失败: {responseCodeResult.Message}");
                }
                DebugLog("[FINS读取调试] 接收到响应码: {ResponseCode}", string.Join(" ", responseCodeResult.ResultValue.Select(b => b.ToString("X2"))));

                // 合并头部和响应码
                var headerAndCode = headerBuffer.Concat(responseCodeResult.ResultValue).ToArray();
                DebugLog("[FINS读取调试] 合并头部和响应码: {HeaderAndCode}", string.Join(" ", headerAndCode.Select(b => b.ToString("X2"))));
                
                // 根据响应码确定数据长度
                var dataLength = FinsCommonMethods.GetDataLength(headerAndCode, request.RegisterCount, request.DataType);
                DebugLog("[FINS读取调试] 计算数据长度: {DataLength}字节", dataLength);
                
                byte[] fullResponse;
                if (dataLength > 0)
                {
                    // 接收数据部分
                    DebugLog("[FINS读取调试] 开始接收数据部分 ({DataLength}字节)", dataLength);
                    var dataResult = await ReceiveResponseAsync(0, dataLength);
                    if (!dataResult.IsSuccess)
                    {
                        DebugLog("[FINS读取调试] 接收数据失败: {Message}", dataResult.Message);
                        return OperationResult.CreateFailedResult<IDeviceReadWriteMessage>($"接收FINS读取响应数据失败: {dataResult.Message}");
                    }
                    DebugLog("[FINS读取调试] 接收到数据: {Data}", string.Join(" ", dataResult.ResultValue.Select(b => b.ToString("X2"))));
                ...
                {
                    // 没有数据部分，只有头部和响应码
                    DebugLog("[FINS读取调试] 无数据部分，只有头部和响应码");
                    fullResponse = headerAndCode;
                }

                DebugLog("[FINS读取调试] 完整响应: {FullResponse}", string.Join(" ", fullResponse.Select(b => b.ToString("X2"))));
                DebugLog("[FINS读取调试] 读取操作完成");
                return OperationResult.CreateSuccessResult<IDeviceReadWriteMessage>(new FinsReadResponse(fullResponse));
            }
            catch (Exception ex)
            {
                DebugLog("[FINS读取调试] 读取操作异常: {Message}", ex.Message);
                DebugLog("[FINS读取调试] 异常堆栈: {StackTrace}", ex.StackTrace);
